    ...
    if config.model.name == "alignn":
        ...
        for e in range(config.epochs):
            ...
            for dats in train_loader:
                ...
                loss = 0
                if config.model.output_features is not None:
                    loss = criterion(result, dats[-1].to(device))
                    info["target_out"] = dats[-1].cpu().numpy().tolist()
                    info["pred_out"] = result.cpu().detach().numpy().tolist()
            ...
            train_mean_loss = get_batch_errors(train_result)
            scheduler.step()
            ...
            val_mean_loss = get_batch_errors(val_result)
            ...
            test_mean_loss = get_batch_errors(test_result)

            history_test.append([test_mean_loss])
